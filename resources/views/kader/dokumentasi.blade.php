...
@section('content')
    <style>
        /* x-cloak untuk sembunyikan elemen sampai Alpine siap */
    ...
    <div>
        ...
        <div>
            <a href="{{ route('kader.dokumentasi.create') }}"
        ...
        <div class="mt-4 bg-cyan-700 shadow-md rounded-lg overflow-x-auto"
            x-data="{ showModal: false, deleteUrl: '', showFullDetails: false, detailItem: {} }">
                    ...
                    @foreach ($dokumentasi as $key => $item)
                        <tr class="hover:bg-gray-100">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @if ($item->fotos->count() > 0)
                                        @foreach ($item->fotos->take(1) as $foto)
                                            <img src="{{ Storage::url($foto->path) }}" alt="Foto Dokumentasi"
                            ...
                            <td class="px-6 py-3 flex justify-center items-center space-x-2 h-full">
                                ...
                                <a href="{{ route('kader.dokumentasi.edit', $item->id) }}"
                                ...
                                <!-- Tombol hapus -->
                                <button
                                    @click="showModal = true; deleteUrl = '{{ route('kader.dokumentasi.destroy', $item->id) }}'"
            ...
            <div x-show="showModal" x-cloak
                ...
                <div
                    class="bg-white rounded-lg shadow-lg p-6 w-[400px] md:w-[500px] max-w-full mx-4
            ...
            <!-- Modal Lihat Detail -->
            <div x-show="showFullDetails" x-cloak
                class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                <div
                    class="bg-white rounded-lg shadow-lg p-6 w-[500px] md:w-[700px] max-w-full mx-4
                           overflow-auto max-h-[90vh]">
                    ...
                    <p class="mb-4 text-blue-800">Waktu: <span
                            x-text="new Date(detailItem.waktu).toLocaleString('id-ID')"></span></p>
                    <p class="mb-4 text-gray-700">Lokasi: <span x-text="detailItem.lokasi"></span></p>
                    <div class="flex flex-wrap gap-2">
                        <template x-for="foto in detailItem.fotos" :key="foto.id">
                            <img :src="'{{ asset('storage/') }}/' + foto.path" alt="Foto"
                                class="w-32 h-32 object-cover rounded-lg shadow">
                        </template>
                    </div>
                    <div class="mt-4 flex justify-end">
                        <button @click="showFullDetails = false"
                            class="px-4 py-2 bg-red-500 text-white rounded-lg">Tutup</button>
                    </div>
                </div>
            </div>
    ...
    <!-- Alpine.js (pastikan ini di layout atau di sini, pakai defer) -->
