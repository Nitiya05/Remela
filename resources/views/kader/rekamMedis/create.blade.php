    ...
    <div class="container mx-auto p-6 max-w-screen-md lg:max-w-screen-lg">
        ...
        <form action="{{ route('rekam-medis-lansia.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" value="{{ $patient->nama }}" class="w-full p-3 border rounded bg-gray-100 text-lg"
                    readonly>
            </div>
            <div>
                <label class="block text-gray-700 text-lg">NIK:</label>
                <input type="text" value="{{ $patient->nik }}" class="w-full p-3 border rounded bg-gray-100 text-lg"
                    readonly>
            </div>
            ...
            <div>
                ...
                <input type="date" name="tanggal_rekam" class="w-full p-3 border rounded text-lg" required
                    value="{{ old('tanggal_rekam') }}">
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <label class="flex items-center space-x-2 text-lg">
                    <input type="checkbox" name="kurang_aktivitas_fisik" value="1"
                        {{ old('kurang_aktivitas_fisik') ? 'checked' : '' }}>
                    <span>Kurang Aktivitas Fisik</span>
                </label>
                <label class="flex items-center space-x-2 text-lg">
                    <input type="checkbox" name="kurang_sayur_buah" value="1"
                        {{ old('kurang_sayur_buah') ? 'checked' : '' }}>
                    <span>Kurang Sayur & Buah</span>
                </label>
                <label class="flex items-center space-x-2 text-lg">
                    <input type="checkbox" name="konsumsi_alkohol" value="1"
                        {{ old('konsumsi_alkohol') ? 'checked' : '' }}>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" step="0.1" id="berat_badan" name="berat_badan"
                        class="w-full p-3 border rounded text-lg" required value="{{ old('berat_badan') }}">
                </div>
                <div>
                    ...
                    <input type="number" step="0.1" id="tinggi_badan" name="tinggi_badan"
                        class="w-full p-3 border rounded text-lg" required value="{{ old('tinggi_badan') }}">
                </div>
                <!-- Lingkar Perut -->
                <div>
                    <label class="block text-gray-700 text-lg">Lingkar Perut (cm):</label>
                    <input type="number" step="0.1" name="lingkar_perut" class="w-full p-3 border rounded text-lg"
                        required value="{{ old('lingkar_perut') }}">
                </div>
            </div>

            <!-- BMI -->
            <div>
                <label class="block text-gray-700 text-lg">Indeks Massa Tubuh:</label>
                <input type="text" id="bmi" name="bmi" class="w-full p-3 border rounded bg-gray-100 text-lg"
                    readonly value="{{ old('bmi') }}">
            </div>

            <!-- Tekanan Darah -->
            <div>
                <label class="block text-gray-700 text-lg mb-1">Tekanan Darah (Sistolik / Diastolik):</label>
                <div class="flex space-x-2 items-center">
                    <input type="number" name="tekanan_darah_sistolik" class="w-1/2 p-3 border rounded text-lg" required
                        value="{{ old('tekanan_darah_sistolik') }}">
                    <span class="text-xl font-bold">/</span>
                    <input type="number" name="tekanan_darah_diastolik" class="w-1/2 p-3 border rounded text-lg" required
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" step="0.1" name="gula_darah" class="w-full p-3 border rounded text-lg"
                        value="{{ old('gula_darah') }}">
                </div>
                <div>
                    <label class="block text-gray-700 text-lg">Kolesterol (mg/dL):</label>
                    <input type="number" step="0.1" name="kolesterol" class="w-full p-3 border rounded text-lg"
                        value="{{ old('kolesterol') }}">
                </div>
                <div>
                    <label class="block text-gray-700 text-lg">Asam Urat (mg/dL):</label>
                    <input type="number" step="0.1" name="asam_urat" class="w-full p-3 border rounded text-lg"
                        value="{{ old('asam_urat') }}">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="obat" class="w-full p-3 border rounded text-lg"
                    value="{{ old('obat') }}">
            </div>
            <div>
            ...
            <div class="flex gap-4">
                <button type="submit"
                    class="bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded text-lg">Simpan</button>
                <a href="{{ route('rekam-medis-lansia.index') }}"
                    class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-3 rounded text-lg">Batal</a>
            </div>
        </form>
    </div>
